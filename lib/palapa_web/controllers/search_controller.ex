defmodule PalapaWeb.SearchController do
  ...
  def put_common_breadcrumbs(conn, _params) do
    put_breadcrumb(conn, gettext("Search"), "#")
  end

  def index(conn, params) do
    if "XMLHttpRequest" in get_req_header(conn, "x-requested-with") do
      search_results = Searches.search(current_member(conn), params["query"], page_size: 5)
      render(conn, "index_ajax.html", layout: false, search_results: search_results)
    else
      search_results =
        Searches.search(current_member(conn), params["query"], page: params["page"], page_size: 15)
