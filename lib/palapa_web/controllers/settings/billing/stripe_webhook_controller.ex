defmodule PalapaWeb.Settings.Billing.StripeWebhookController do
  ...
  def create(
    ...
    customer = Billing.get_customer_by_stripe_id!(stripe_subscription.customer)

    case Billing.update_customer_subscription_status(customer, stripe_subscription.status) do
      {:ok, customer} ->
        send_resp(conn, :ok, "Updated subscription status : #{customer.subscription_status}")
