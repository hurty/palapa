...
export default class extends BaseController {
  ...
  displayFields() {
    if (this.privateCheckboxTarget.checked) this.show(this.visibilitiesTarget);
  ...
  clearForm() {
    this.element.querySelectorAll(".input").forEach((node, index) => {
      node.value = null;
    });
    this.element.querySelectorAll(".error").forEach((node, index) => {
      node.remove();
    });

    if (typeof this.dropzone !== "undefined") this.dropzone.removeAllFiles();
  }
