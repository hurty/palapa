...
defmodule PalapaWeb.AuthenticationTest do
  ...
  @registration %{email: "pierre.hurtevent@gmail.com", name: "Pierre Hurtevent", password: "password", organization_name: "PalapaCorp"}
  ...
  test "call places the current user into assigns", %{conn: conn} do
    {:ok, %{user: user, organization: organization}} = Palapa.Accounts.create_registration(@registration)
  ...
  test "login with valid email and password", %{conn: conn} do
    {:ok, %{user: user}} = Palapa.Accounts.create_registration(@registration)
    {:ok, conn} = Authentication.login_with_email_and_password(conn, "pierre.hurtevent@gmail.com", "password")
    assert conn.assigns.current_user.id == user.id
  end
  ...
  test "login with password mismatch", %{conn: conn} do
    ...
    assert {:error, :unauthorized, _conn} =
      Authentication.login_with_email_and_password(conn, "pierre.hurtevent@gmail.com", "wrong_pass")
  end
end